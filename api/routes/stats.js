...
router.post("/auditlogs", async (req, res) => {
    try {
        ...
        let result = await AuditLogs.aggregate([//aggregate sql deki group by veya join gibi düşünülebilir
            { $match: filter },
...
router.post("/users/count", async (req, res) => {
    try {
        ...
        let result = await Users.countDocuments(filter);//Users.count fonksiyonu yerine Users.countDocuments fonksiyonunu kullanarak belge sayısını aldık.
